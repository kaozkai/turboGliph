...
#'
#' @description With this method the scores of cdr3 clusters are calculated as 
#' in the GLIPH and GLIPH2 algorithm.
#' Depending on the information provided, a final score is calculated based on 
#' up to five cluster properties:
#' cluster size, enrichment of cdr3 lengths, enrichment of V genes, enrichment 
#' of clonal expansions and enrichment of a common HLA alleles.
#' @param cluster_list list. Each element of this list contains a data frame in 
#' which the CDR3b sequences and additional information necessary for
#' scoring are provided. Corresponds to the \code{$cluster_list} element of the 
#' output of the functions \code{turbo_gliph} and \code{gliph2}.
...
#' If \code{type = "paired chains"} columns named "CDR3a" (alpha sequences) and 
#' "CDR3b" (beta sequences) are required.
...
#' databases were provided for download by the developers of GLIPH2 in the 
#' web tool (http://50.255.35.37:8080/tools).
...
#' contains the frequency of CDR3 lengths in a naive T cell repertoire 
#' required for scoring. The first column provides the CDR3 lengths and the 
#' second column the frequencies.  If the value is \code{NULL}, default 
...
#' @param gliph_version numeric. Either \code{1} for GLIPH or \code{2} for 
#' GLIPH2 algorithm. The scoring of the algorithms differs only in the 
#' calculation of the total score. GLIPH2 calculates the product of all 
#' individual scores, while GLIPH multiplies this product additionally by 0.064.
#' @param sim_depth numeric. By default 1000. Simulated resampling depth for 
#' non-parametric convergence significance tests. A higher number will take 
#' longer to run but will produce more reproducible results.
...
#'
#' @return This function produces one file in the \code{result_folder} named 
#' \code{"GLIPH_scoring_results.txt"}
#' containing the same information as in the returned data frame.
#' The data frame contains the cluster scoring results. The first columns 
#' provides the \code{representative_seq} for any evaluated cluster.
#' In the second column the total scores are stored. Additional columns contain 
#' up to five scores (cluster size, cdr3 length enrichment, V-gene enrichment,
#' enrichment of clonal expansion and enrichment of common HLA) used to evaluate
#' the total score.
#'
#' @examples
...
cluster_scoring <- function(cluster_list,
  ...
  if(!base::is.list(cluster_list)) 
    base::stop("parameter 'cluster_list' has to be an object of class 'list'.")
  
  ### cdr3_sequences
  if(base::is.atomic(cdr3_sequences)) 
    cdr3_sequences <- base::data.frame("CDR3b" = cdr3_sequences)
  if(!base::is.data.frame(cdr3_sequences)) 
    base::stop("parameter 'cdr3_sequences' has to be an object of class 
               'data.frame'.")
  ...
  if(!base::is.null(v_usage_freq)){
    if(base::is.data.frame(v_usage_freq)){
      ...
      base::stop("v_usage_freq has to be a data frame containing V-gene 
                 information in the first column and the corresponding frequency
  ...
  if(!base::is.null(cdr3_length_freq)){
    if(base::is.data.frame(cdr3_length_freq)){
      ...
      base::stop("cdr3_length_freq has to be a data frame containing CDR3 
                 lengths in the first column and the corresponding frequency in 
                 a naive T-cell repertoire in the second column.")}
  }
  ...
  ### gliph_version
  if(!(gliph_version %in% base::c(1,2))) 
    base::stop("gliph_version has to be either 1 or 2.")
  ...
  if("patient" %in% base::colnames(cdr3_sequences)) 
    patient_info <- TRUE else patient_info <- FALSE
  if("HLA" %in% base::colnames(cdr3_sequences)) 
    hla_info <- TRUE else hla_info <- FALSE
  if(hla_info == TRUE && patient_info == TRUE){
    if("patient" %in% base::colnames(cdr3_sequences) && 
       "HLA" %in% base::colnames(cdr3_sequences)){
      cdr3_sequences <- cdr3_sequences[cdr3_sequences$HLA != "" & 
                                         !base::is.na(cdr3_sequences$HLA),]
      if(nrow(cdr3_sequences > 0)) 
        score_names <- base::c(score_names, "hla.score", "lowest.hlas") 
      else hla_info <- FALSE
    }
  }

  ### load or generate reference tables from reference database
  # ref_cluster_sizes:            data frame containing the cluster size in the 
  #                               first and the probability of observing a 
  #                               cluster with this size
  #                               in a sample from the reference database in the
  #                               second column
  # vgene_ref_frequencies:        vector containing the (relative) frequencies 
  #                               of v gene usage
  # cdr3_length_ref_frequencies:  vector containing the (relative) frequencies 
  #                               of CDR3b lengths
  utils::data(ref_cluster_sizes, envir = base::environment(), 
              package = "turboGliph")
  ...
  if(!base::identical(refdb_beta, "gliph_reference")){
    if("TRBV" %in% base::colnames(refdb_beta)){
      vgene_ref_frequencies <- base::as.data.frame(
        base::table(refdb_beta$TRBV))
      vgene_ref_frequencies <- vgene_ref_frequencies$Freq/
        sum(vgene_ref_frequencies$Freq)
    }
    cdr3_length_ref_frequencies <- base::as.data.frame(base::table(base::nchar(
      refdb_beta$CDR3b)))
  ...
  if(!base::is.null(v_usage_freq)) 
    vgene_ref_frequencies <- base::as.numeric(v_usage_freq[,2])
  if(!base::is.null(cdr3_length_freq)) 
    cdr3_length_ref_frequencies <- base::as.numeric(cdr3_length_freq[,2])
  ...
  # all_patient_hlas: list whose elements are named after the patients and 
  #                   contain the patient's HLA alleles in a vector.
  ...
  if(hla_info == TRUE && patient_info == TRUE){
    ...
    all_hlas <- base::sort(base::unique(base::gsub(":.*", "", all_hlas, 
                                                   perl = TRUE)))
    num_patients <- base::length(all_patients)
    num_HLAs <- base::length(all_hlas)

    all_patient_hlas <- base::lapply(all_patients, function(x){
      base::sort(base::unique(base::gsub(":.*", "", base::unlist(
        base::strsplit(cdr3_sequences$HLA[
          cdr3_sequences$patient == x][1], ",")), perl = TRUE)))
    })
