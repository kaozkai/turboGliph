...
#'
#' @description Finding motifs in sequences. This function searches and 
#' simultaneously counts continuous and discontinuous motifs in a sequence 
#' vector.
#' It is used in the \code{turbo_gliph} function to identify local similarities.
#' @param seqs character vector. This vector must contain the sequences whose 
#' motifs are to be identified and quantified.
...
#' discontinuous motifs are to be considered.
#'
#' @return \code{find_motifs} returns a data frame with two columns.
#' The first column contains the motifs and the second column the frequency of 
#' the motifs.
#'
#' @examples
...
#'
#' @export
find_motifs <- function(seqs, q = 2:4,kmer_mindepth = NULL, 
                        discontinuous = FALSE){
  all_kmers <- base::c()
  seqs <- base::as.character(seqs)
  all_q <- q
  if(discontinuous == TRUE) all_q <- base::sort(base::unique(
    base::c(all_q, q+1)))
  ...
  for(i in all_q){
    ...
    if(!base::is.null(kmer_mindepth)) cont_kmer <- cont_kmer[base::which(
      cont_kmer$V1 >= kmer_mindepth),] 
    #dplyr::filter(cont_kmer,(V1 >= kmer_mindepth))
    ...
    if(discontinuous == TRUE && i %in% (q+1)) {
      ## Find all discontinuous motifs with (i-1) fixed positions and variable 
      ## position j
      for(j in 2:(i-1)) {
        ## Replace position j by a dot in all continuous motifs  with length i
        disc_kmer <- cont_kmer
        base::substr(disc_kmer$motif, j, j) <- "."
        disc_kmer <- base::as.data.frame(base::table(base::rep(disc_kmer$motif, 
                                                               disc_kmer$V1)))
        base::colnames(disc_kmer) <- base::c("motif", "V1")

        ## Exclude motifs with less counts than kmer_mindepth
        if(!base::is.null(kmer_mindepth)) disc_kmer <- disc_kmer[
          base::which(disc_kmer$V1 >= kmer_mindepth),] 
        #dplyr::filter(disc_kmer,(V1 >= kmer_mindepth))
