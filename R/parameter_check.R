...
parameter_check <- function(...){
  ...
  p$save_results <- ch_result_folder(p$result_folder, p$sim_depth, p$lcminp,
                                       p$lcminove)
  ch_refdb_beta(p$refdb_beta)
  ch_v_usage_freq(p$v_usage_freq)
  ch_cdr3_length_freq(p$cdr3_length_freq)
  ch_ref_cluster_size(p$ref_cluster_size)
  p$sim_depth <- ch_sim_depth(p$sim_depth)
  ch_lcminp(p$lcminp)
  ch_lcminove(p$lcminove, p$motif_length)
  ch_kmer_mindepth(p$kmer_mindepth)
  ch_accept_sequences_with_C_F_start_end(p$accept_sequences_with_C_F_start_end)
  p$min_seq_length <- ch_min_seq_length_a(p$min_seq_length)
  ch_gccutoff(p$gccutoff)  
  ch_structboundaries(p$structboundaries)
  l <- ch_boundary_size(p$boundary_size, p$structboundaries, p$min_seq_length)
  p$boundary_size <- l[[1]]
  p$min_seq_length <- l[[2]]
  ch_motif_length(p$motif_length)
  ch_discontinuous(p$discontinuous)
  ch_make_depth_fig(p$make_depth_fig)
  ch_local_similarities(p$local_similarities)
  ch_global_similarities(p$global_similarities, p$local_similarities)
  ch_global_vgene(p$global_vgene)
  ch_positional_motifs(p$positional_motifs)
  ch_cdr3_len_stratify(p$cdr3_len_stratify)
  ch_vgene_stratify(p$vgene_stratify)
  ch_public_tcrs(p$public_tcrs)
  p$cluster_min_size <- ch_cluster_min_size(p$cluster_min_size)
  ch_hla_cutoff(p$hla_cutoff)
  p$n_cores <- ch_n_cores(p$n_cores)
  p$min_seq_length <- ch_min_seq_length_b(p$structboundaries, p$min_seq_length, 
                                        p$boundary_size)
  return(p)
}
